...
app.post('/game_end', (req, res) => {
    const user_id = req.body.nickname
	...
	try {
	    let connection = await pool.getConnection(async conn => conn)
            let [row] = await connection.query(callGameEnd, [user_id, score]);
	    connection.release();
	    console.log('이것이 row 값이다');
	    res.send(row);
	} catch(err) {
	    console.log(err)
	    res.status(500).send('game_end오류');
	}
    }
    gameend();
})
...
app.get("/show_rank", (req, res) => { // GET(데이터 보내지 않고 얻어 오기만 하는 거고) , POST (데이터를 보내서 생성하면서 얻어 올수있다)
  ...
  app.post('/datasave', (req, res) => {
    ...
    console.log(obj.character_level[30001])
    ...
    const datasave = async () => {
      try {
        let connection = await pool.getConnection(async conn => conn)
        let [row] = await connection.query(save_user, [nickname, coin, highscore, nickname])
	connection.release();
        let [row1] = await connection.query(save_skillcharactor, [기사, 법사, 연속찌르기, 돌진, 회전공격, 내려찍기, 검기날리기, 연속베기, 밀쳐내기, 흘리기, 회복, nickname])
        connection.release()
